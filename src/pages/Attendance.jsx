  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center justify-between mb-1">
        <h1 className="font-display text-3xl font-semibold text-ledger-navy">Check in</h1>
        ...
        <div className="mt-4 flex gap-3">
          {!photo ? (
            <button
              ...
              className="flex-1 bg-ledger-navy text-white rounded py-2.5 text-sm font-medium hover:bg-ledger-navyDeep disabled:opacity-50"
            >
              Capture photo
            </button>
          ) : (
            <button
              onClick={retake}
              className="flex-1 border border-ledger-line rounded py-2.5 text-sm font-medium hover:bg-ledger-chalk"
            >
              Retake
            </button>
          )}
      ...
      <div className="badge-card ml-2 p-5 mb-6">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-ledger-navy">Location</p>
            {location ? (
              <p className="text-xs font-mono text-ledger-navy/60 mt-1">
                {location.lat.toFixed(5)}, {location.lng.toFixed(5)} · ±
                {Math.round(location.accuracy)}m
                {SCHOOL_COORDS && (
                  <span className={withinGeofence ? "text-ledger-sage" : "text-ledger-brick"}>
                    {" "}
              ...
              <p className="text-xs text-ledger-navy/40 mt-1">Not shared yet</p>
            )}
          </div>
          <button
            onClick={fetchLocation}
            className="text-sm font-medium text-ledger-marigold hover:underline"
          >
      ...
      <button
        ...
        className="w-full bg-ledger-sage text-white rounded py-3 text-sm font-medium hover:opacity-90 disabled:opacity-40"
      >
        {saveStatus === "saving"
          ? "Saving…"
          : saveStatus === "done"
